    ...
    return (
        <section className="bg-white py-4 mt-2">
            <div className="px-4 flex items-center justify-between mb-3">
                ...
                <Link href={href} className="text-primary text-xs font-bold uppercase tracking-wider flex items-center gap-1 group">
                    See All <ChevronRight size={14} className="group-hover:translate-x-1 transition-transform" />
                </Link>
            </div>
            {/* Horizontal Scroll for Mobile */}
            <div className="pl-4 pr-1 pb-4 flex overflow-x-auto gap-4 no-scrollbar snap-x">
                {products.map((p) => (
                    <div key={p.id} className="w-[140px] flex-shrink-0 snap-start">
                        <ProductCard product={p} className="h-full border-none shadow-none bg-transparent" />
                    </div>
                ))}
            </div>
        </section>
...
export default async function HomePage() {
    ...
    return (
        <div className="bg-gray-100 min-h-screen pb-24">
            ...
            <div className="bg-primary px-4 py-3 sticky top-0 z-30 shadow-sm safe-top">
                ...
                <Link href="/search" className="bg-white rounded-xl py-2.5 px-3 flex items-center gap-2 text-gray-400 text-sm shadow-inner transition-opacity active:opacity-80">
                    <span className="opacity-70">🔍</span> Search products, cosmetics, jewelry...
                </Link>
            </div>
            ...
            {/* ── Categories ── */}
            <section className="bg-white pt-5 pb-4 mt-2">
                ...
                {categories.length === 0 ? (
                    ...
                    <div className="grid grid-cols-4 gap-y-6 gap-x-2 px-3">
